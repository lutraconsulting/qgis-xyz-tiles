...
# TMS functions taken from https://alastaira.wordpress.com/2011/07/06/converting-tms-tile-coordinates-to-googlebingosm-tile-coordinates/
def tms(ytile, zoom):
    n = 2.0 ** zoom
    ytile = n - ytile - 1
    return int(ytile)
...
class DirectoryWriter:
    def __init__(self, folder, tile_params, is_tms):
        self.folder = folder
...
class TilesXYZAlgorithm(QgisAlgorithm):
    EXTENT = 'EXTENT'
    ZOOM_MIN = 'ZOOM_MIN'
    ZOOM_MAX = 'ZOOM_MAX'
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if output_format == 'Directory':
            writer = DirectoryWriter(output_dir, tile_params, is_tms)
        else:
        ...
        if output_html and output_format == 'Directory':
            html_code = LEAFLET_TEMPLATE.format(
                ...
                centerx = (wgs_extent[2] - wgs_extent[0])/2,
                centery = (wgs_extent[3] - wgs_extent[1])/2,
